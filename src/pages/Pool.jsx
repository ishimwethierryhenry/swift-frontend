    ...
    <div className="w-full min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 overflow-hidden relative">
      ...
      <div className={`flex flex-row items-center justify-between w-full h-20 py-4 px-8 backdrop-blur-lg bg-white/10 border-b border-white/20 transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
        <div className="flex flex-row justify-center items-center group">
          <div className="w-12 h-12 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full flex items-center justify-center mr-3 group-hover:scale-110 transition-transform duration-300">
            <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center">
              <div className="w-4 h-4 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full animate-pulse"></div>
            </div>
          </div>
          <label className="font-bold text-3xl text-white tracking-wide">SWIFT</label>
        </div>
        ...
        <div className="px-8 py-8 max-w-7xl mx-auto space-y-6 md:space-y-8 pb-20">
          ...
          <div className={`text-center mb-8 transition-all duration-1000 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
            <div className="overflow-hidden">
              <label className={`font-bold text-4xl text-white block transition-all duration-1000 delay-500 ${isVisible ? 'translate-y-0' : 'translate-y-full'}`}>
                Real-Time Pool Monitor
              </label>
            </div>
            <div className="overflow-hidden">
              <label className={`font-bold text-xl text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-400 block mt-2 transition-all duration-1000 delay-700 ${isVisible ? 'translate-y-0' : 'translate-y-full'}`}>
          ...
          {/* Parameter Cards */}
          <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 transition-all duration-1000 delay-900 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            ...
            <div className={`relative group transition-all duration-700 delay-1000 ${isVisible ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}`}>
              <div className={`absolute -inset-4 rounded-3xl opacity-30 group-hover:opacity-40 transition-opacity duration-300 blur-lg ${
                sensorData?.ph > 7.1 && sensorData?.ph < 7.3 
                  ? "bg-gradient-to-r from-emerald-500 to-teal-500" 
                  : "bg-gradient-to-r from-red-500 to-orange-500"
              }`}></div>
              <div className={`relative backdrop-blur-lg bg-white/10 rounded-3xl p-6 border-2 transition-all duration-300 ${
                sensorData?.ph > 7.1 && sensorData?.ph < 7.3 
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <label className="font-bold text-5xl text-cyan-300">
            ...
            {/* Conductivity Card */}
            <div className={`relative group transition-all duration-700 delay-1200 ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
              <div className={`absolute -inset-4 rounded-3xl opacity-30 group-hover:opacity-40 transition-opacity duration-300 blur-lg ${
                sensorData?.tds < 2001 
                  ? "bg-gradient-to-r from-emerald-500 to-teal-500" 
                  : "bg-gradient-to-r from-red-500 to-orange-500"
            ...
            <div className="relative backdrop-blur-lg bg-white/10 rounded-3xl p-8 border border-white/20 hover:border-white/40 transition-all duration-300">
              ...
              <div className="flex flex-col gap-6">
                <div className="text-center">
                  <label className="font-bold text-3xl text-white">Water Parameters Analytics</label>
                  <p className="text-gray-300 mt-2">Real-time data visualization and trends</p>
                </div>
                ...
                {maintainancePrediction && (
                  <div className="bg-gradient-to-r from-purple-500/20 to-pink-500/20 rounded-2xl p-6 border border-purple-400/30">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-bold text-xl text-white">Maintenance Prediction</h3>
                      ...
                      <button
                        ...
                        className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 disabled:from-gray-500 disabled:to-gray-600 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-lg shadow-purple-500/25 disabled:cursor-not-allowed disabled:transform-none"
                      >
